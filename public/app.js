...
function updateSecurityInfo() {
    ...
    switch (securityLevel) {
        case 'maximum':
            info = `<strong>🛡️ Maximum Security:</strong> Full Winternitz verification with transaction binding. 
                    Signature is mathematically bound to the specific transaction - front-running is <em>impossible</em>. 
                    Larger script (~3KB), higher fees (~3000 sats).`;
            className = 'info-box info-maximum';
            break;
        case 'enhanced':
            info = `<strong>🔐 Enhanced Security:</strong> Improved preimage verification with optional time-lock. 
                    Good balance of security and efficiency. Medium script size, moderate fees.`;
            className = 'info-box info-enhanced';
            break;
        case 'standard':
        default:
            info = `<strong>⚡ Standard Security:</strong> Simple preimage check. Quantum-resistant for storage. 
                    Theoretically vulnerable to mempool front-running during spend (low practical risk). 
                    Smallest script (~35 bytes), lowest fees (~35 sats).`;
